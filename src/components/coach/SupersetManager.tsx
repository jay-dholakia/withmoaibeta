...
import { Trash, Plus, MoveVertical, X } from 'lucide-react';
import { toast } from "sonner";
import { 
  createSupersetGroup, 
  addExerciseToSupersetGroup,
...
const SupersetManager: React.FC<SupersetManagerProps> = ({
  workoutId,
  exercises,
  onSupersetCreated,
  onError
  ...
  const handleCreateSuperset = async () => {
    ...
    try {
      ...
      const supersetGroup = await createSupersetGroup({
      ...
      for (let i = 0; i < selectedExercises.length; i++) {
        await addExerciseToSupersetGroup(selectedExercises[i], supersetGroup.id, i);
      }
